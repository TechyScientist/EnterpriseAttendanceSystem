...
@NamedQueries({
        @NamedQuery(name="Course.Lookup", query="SELECT c FROM Course c WHERE LOWER(c.term) = :term AND LOWER(c.subject) = :subject AND c.number = :number AND LOWER(c.section) = :section"),
        @NamedQuery(name="Course.FindByTerm", query="SELECT c FROM Course c WHERE LOWER(c.term) = :term"),
        @NamedQuery(name="Course.FindSectionsByCourseNumber", query="SELECT c FROM Course c WHERE LOWER(c.subject) = :subject AND c.number = :number"),
